...
function IconTrophy() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      <polyline points="8 6 2 6 2 12 8 18"/>
      <polyline points="16 6 22 6 22 12 16 18"/>
      <path d="M12 18v4"/>
      <path d="M8 22h8"/>
      <path d="M8 6h8l-1 10H9L8 6z"/>
    </svg>
  );
}
...
function DashboardPage(props) {
  ...
  const appliedCount = applications.filter(function(job) {
    return job.status === 'Applied';
  }).length;
  const interviewCount = applications.filter(function(job) {
    return job.status === 'Interview';
  }).length;
  const offerCount = applications.filter(function(job) {
  ...
  const pipelineSegments = pipeline.map(function(seg) {
    const segmentStyle = {
      '--segment-width': seg.pct + '%',
      '--segment-bg': seg.bg
    };
  ...
  const pipelineLegendItems = pipeline.map(function(seg) {
    const dotStyle = {
      '--legend-color': seg.color
    };

    return (
      <div key={seg.label} className="dash-legend-item">
        <div className="dash-legend-dot" style={dotStyle} />
        {seg.label} ({seg.count})
      </div>
    );
  });

  const recentApplicationRows = recentApps.map(function(app) {
    return (
  ...
  const actionItemRows = actionItems.map(function(item) {
    const dotStyle = {
    ...
    return (
      <div key={item.id} className="dash-todo-row">
        <div className="dash-todo-dot" style={dotStyle} />
        <div>
  ...
  const insightItems = quickInsights.map(function(stat) {
    return (
  ...
  return (
    <PageLayout>
        ...
        <section className="card">
          ...
          {isEmpty ? (
            <div className="dash-empty-state">
              <IconBriefcase />
              <p className="dash-empty-message">
                No applications yet.<br />Start by saving jobs you&apos;re interested in.
              </p>
        ...
        <section className="card">
          ...
          {actionItems.length === 0 ? (
            <div className="dash-empty-state">
              <IconCheckCircle />
              <p className="dash-empty-message">
                No action items right now.<br />You&apos;re all caught up!
              </p>
      ...
      <section className="card">
        <div className="dash-section-header dash-section-header-spaced">
